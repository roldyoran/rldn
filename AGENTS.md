...
Canvas App - infinite canvas for images/diagrams with auth. Monorepo managed by Turborepo + Bun workspaces. Designed for multi-feature extensibility (canvas, kanban, todos, notes, etc.).
...
```
apps/web/              → Astro 6 (SSR, Bun adapter, TailwindCSS 4 + Starwind UI)
  src/
    pages/             → Astro file-based routing (thin controllers)
    layouts/           → HTML shell (Layout.astro)
    features/          → Feature modules (self-contained)
      canvas/          → Excalidraw canvas (React island)
        components/    → CanvasPage.tsx, ExcalidrawEditor.tsx, ColorPickerPopup.tsx, ImageUrlDialog.tsx
      dashboard/       → Dashboard UI (Astro components)
        components/    → TopBar, CanvasGrid, DeleteModal, ApiKeyModal, Icons, etc.
      kanban/          → (future) Kanban boards
      todos/           → (future) Todo lists
      notes/           → (future) Notes
    shared/            → Cross-feature shared code
      lib/             → api-auth.ts, auth.ts, auth-client.ts, db.ts
    styles/            → starwind.css (Starwind UI theme), global.css (Excalidraw overrides)
    components/starwind/ → Auto-generated by Starwind CLI (do not edit manually)
...
`pages/ → features/ → shared/`. Never the reverse. Features never import from other features.

### UI Component Library

**Starwind UI** — shadcn/ui for Astro. Components are `.astro` files + vanilla JS, not React.
- Installed via CLI: `npx starwind@latest add <component>`
- Components live in `src/components/starwind/` (auto-generated, own the code)
- Theme via CSS variables in `src/styles/starwind.css`
- 49+ components: Button, Card, Dialog, Alert Dialog, Sidebar, Tabs, etc.
- For interactive React features (canvas, kanban drag-and-drop), use React islands with Starwind as the UI shell
...
- `apps/web/src/shared/lib/db.ts` uses `import.meta.env.TURSO_DB_URL || import.meta.env.DATABASE_URL` (Vite-injected)
...
- `documents` table is the base entity for all features (type discriminator: 'canvas' | 'kanban' | 'todo' | 'note')
- Feature-specific data stored in type-specific tables (1:1 with documents via documentId)
...
- `ExcalidrawEditor` component (`apps/web/src/features/canvas/components/ExcalidrawEditor.tsx`) handles data loading internally via fetch, passes data as `initialData` on first render
